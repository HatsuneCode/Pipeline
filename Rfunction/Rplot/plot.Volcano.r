...
## df need: avg_log2FC, p_val_adj, gene
plot_FP = function(df, FC.col = 'avg_log2FC', logFC = 1, padj = .01, label.logFC = 1.5, exprAvg = 0, 
                   xlab = 'Log2 Fold Change', title = 'DEG Log2FC-Padj', 
                   adj = T, pmax = 1e-300, 
                   label = F, label.name = NULL, label.color = 'red',
                   max.overlaps = 10, font.family = 'Arial') {
  suppressMessages(library(ggplot2))
  suppressMessages(library(ggrepel))
  if (!adj) df$p_val_adj = df$p_val
  ## annot
  df$annot = ifelse(df[[FC.col]] > logFC, 'Up', ifelse(df[[FC.col]] < -logFC, 'Down', 'NS'))
  ...
  df$lable = if (label) if (length(label.name)) df$gene %in% label.name else 
    abs(df[[FC.col]]) > label.logFC & !grepl('^NS', df$annot) else NA
  ## color
  ...
  ## limit
  lim = max(abs(df[[FC.col]]))
  df$p_val_adj[ -log10(df$p_val_adj) > -log10(pmax) ] = pmax
  df$plotFP = df[[FC.col]]
  ## plot
  p = ggplot(df, aes(plotFP, -log10(p_val_adj))) + 
    ...
    geom_text_repel(label = ifelse(df$lable, df$gene, NA), family = font.family, 
