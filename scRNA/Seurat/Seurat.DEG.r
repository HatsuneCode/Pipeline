## Seurat: DEG
Seurat.DEG = function(obj, pos = NULL, neg = NULL, group.by = 'seurat_clusters', 
                      min.pct = .01, logfc = 0, min.cell = 3, max.cells.per.ident = Inf,
                      assay = 'SCT', slot = 'data', method = 'wilcox_limma', 
                      only.pos = F, pre = F, meta.calcu = T) {
  ...
  ## gene meta
  mk$gene = rownames(mk)
  meta.n = c('average', 'median', 'posAvg', 'posMed', 'negAvg', 'negMed')
  if (meta.calcu) {
    ...
    meta  = setNames(data.frame(cbind(
      rowMeans(expr),  apply(expr,  1, median), 
      rowMeans(exprP), apply(exprP, 1, median),
      rowMeans(exprN), apply(exprN, 1, median) )), meta.n)
  } else 
    meta = setNames(data.frame(matrix(NA, nrow = nrow(mk), ncol = length(meta.n))), meta.n)
  mk = cbind(mk, meta)
  ## type meta
  mk$type    = name
